...
var _noProfanity = require("no-profanity");
var _constants = require("../../../shared/config/constants.js");
...
function initializeUser(User) {
  ...
  User.prototype.mobileLoginByRequest = function mobileLoginByRequest(req, res) {
    return new Promise((resolve, reject) => this.createAccessToken({}, (err, accessToken) => {
      if (err) {
        return reject(err);
      }
      (0, _getSetAccessToken.setAccessTokenToResponse)({
  ...
  User.doesExist = function doesExist(username, email) {
    ...
    if (username && (_constants.blocklistedUsernames.includes(username) || (0, _noProfanity.isProfane)(username))) {
  ...
  function requestAuthEmail(isSignUp, newEmail) {
    return _rx.Observable.defer(() => {
      ...
      type: 'info',
      message: (0, _dedent.default)`Check your email and click the link we sent you to confirm your new email address.`
    });
  }
  ...
  User.getPublicProfile = function getPublicProfile(username, cb) {
    return User.findOne$({
      ...
      const allUser = _objectSpread(_objectSpread(_objectSpread(_objectSpread({}, _lodash.default.pick(user, _publicUserProps.publicUserProps)), {}, {
        ...
        completedChallenges
      }, (0, _publicUserProps.getProgress)(progressTimestamps)), (0, _publicUserProps.normaliseUserFields)(user)), {}, {
  ...
  User.prototype.getCompletedExams$ = function getCompletedExams$() {
    if (Array.isArray(this.completedExams) && this.completedExams.length) {
      return _rx.Observable.of(this.completedExams);
    }
    const id = this.getId();
    const filter = {
      where: {
