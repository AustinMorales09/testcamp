...
exports.createExamResults = createExamResults;
exports.generateRandomExam = generateRandomExam;
...
function generateRandomExam(examJson) {
  const {
  ...
  const randomizedExam = examQuestions.map(question => {
    ...
    const answers = shuffleArray([correctAnswer, ...availableWrongAnswers.slice(0, numberOfAnswersPerQuestion - 1)]);
    return {
      id: question.id,
...
function createExamResults(userExam, originalExam) {
  const {
    userExamQuestions,
    examTimeInSeconds
  } = userExam;
  ...
  const numberOfCorrectAnswers = userExamQuestions.reduce((count, userQuestion) => {
    const originalQuestion = originalQuestions.find(examQuestion => examQuestion.id === userQuestion.id);
    if (!originalQuestion) {
      throw new Error('An error occurred. Could not find exam question.');
    }
    const isCorrect = originalQuestion.correctAnswers.find(examAnswer => examAnswer.id === userQuestion.answer.id);
