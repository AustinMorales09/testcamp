...
const {
  availableLangs
} = require('../../../../shared/config/i18n');
function getReturnTo(encryptedParams, secret, _homeLocation = process.env.HOME_LOCATION) {
  let params;
  try {
...
function normalizeParams({
  ...
  if (!returnTo || !allowedOrigins.some(allowed => returnTo.startsWith(allowed + '/'))) {
    returnTo = `${_homeLocation}/learn`;
    origin = _homeLocation;
    pathPrefix = '';
  }
