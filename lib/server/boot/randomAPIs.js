...
module.exports = function (app) {
  ...
  router.get('/api/github', _disabledEndpoints.deprecatedEndpoint);
  ...
  function unsubscribeDeprecated(req, res) {
    req.flash('info', 'We are no longer able to process this unsubscription request. ' + 'Please go to your settings to update your email preferences');
    const {
      origin
  ...
  function unsubscribeById(req, res, next) {
    ...
    return User.find({
      ...
      return Promise.all(updates).then(() => {
        req.flash('success', "We've successfully updated your email preferences.");
        return res.redirectWithFlash(`${origin}/unsubscribed/${unsubscribeId}`);
      }).catch(next);
    });
  }
  function resubscribe(req, res, next) {
    ...
    if (!unsubscribeId) {
      req.flash('info', 'We we unable to process this request, please check and try again');
      res.redirect(origin);
    }
    return User.find({
      where: {
      ...
      return new Promise((resolve, reject) => user.updateAttributes({
        ...
        req.flash('success', "We've successfully updated your email preferences. Thank you for resubscribing.");
        return res.redirectWithFlash(origin);
      }).catch(next);
    });
  }
