...
exports.devSaveResponseAuthCookies =
  exports.devLoginRedirect =
  exports.createPassportCallbackAuthenticator =
    void 0;
...
var _i18n = require("../../configs/i18n");
var _secrets = require("../../configs/secrets");
var _passportProviders = _interopRequireDefault(
  require("./passport-providers")
);
...
_loopbackComponentPassport.PassportConfigurator.prototype.init =
  function passportInit(noSession) {
...
function setupPassport(app) {
  ...
  Object.keys(_passportProviders.default).map(function (strategy) {
    ...
    configurator.configureProvider(
      strategy,
      _objectSpread(_objectSpread({}, config), passportOptions)
    );
  });
}
...
const devLoginRedirect = () => {
  return (req, res) => {
    ...
    let { returnTo, origin, pathPrefix } = (0, _redirection.getRedirectParams)(
      req,
      ({ returnTo, origin, pathPrefix }) => {
        pathPrefix = _i18n.availableLangs.client.includes(pathPrefix)
          ? pathPrefix
          : "";
        return {
          returnTo,
    ...
    const redirectBase = (0, _redirection.getPrefixedLandingPath)(
      origin,
      pathPrefix
    );
    returnTo += (0, _redirection.haveSamePath)(redirectBase, returnTo)
      ? "/learn"
      : "";
...
const createPassportCallbackAuthenticator =
  (strategy, config) => (req, res, next) => {
    return _passport.default.authenticate(
      strategy,
      {
        session: false,
      },
      (err, user, userInfo) => {
        ...
        let { returnTo, origin, pathPrefix } = (0, _redirection.getReturnTo)(
          state,
          _secrets.jwtSecret
        );
        const redirectBase = (0, _redirection.getPrefixedLandingPath)(
